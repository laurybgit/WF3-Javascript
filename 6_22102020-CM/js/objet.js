...
// Pourriez-vous faire en sorte de changer le contenu du titre "J'ai changé le titre" lorsqu'on clique dessus ?
titre.addEventListener("click", function (toto){
    console.log("toto="+toto); // (evenement : mouseEvent)
    console.log("toto.target="+toto.target); // target permet de récupérer la cible de l'objet cliqué l'elt html
    console.log("toto.type="+toto.type); // type d'evenement (Click)
    toto.target.innerHTML="J'ai changé le titre";
});

// Faisons enfin une détection de déplacement souris, et affichons ses coordonnées (dans le p du blockquote): 
let pCoordonnees = document.querySelector('blockquote p'); // mettre le paragraphe dans la variable pCoordonnees
document.addEventListener("mousemove", function(e){
    pCoordonnees.innerHTML="Position en x="+e.clientX+"px<br> Position en y="+e.clientY+"px<br/>test pour Laury :)";
});

//  en passant, on peut accéder aux propriétés css d'un objet via la propriété style, suivi de la prop css... :
